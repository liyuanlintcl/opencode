...
// `${BRAND}.settings.dat` is special: it maps to the `${BRAND}.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"${BRAND}.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // `${BRAND}.settings.dat` → the electron settings store (`${BRAND}.settings`).
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", `${BRAND}.global.dat`).
  const storeName = filename === `${BRAND}.settings.dat` ? `${BRAND}.settings` : filename
