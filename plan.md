...
| T16 | 安装/更新冲突处理（已存在时提示覆盖） | 交互式确认，支持 `--force` 静默覆盖 | 2h | ⏳ |
| T17 | 下载进度条显示 | 大文件下载时有视觉反馈 | 2h | ⏳ |
...
| # | 任务 | 验收标准 | 预估 | 状态 |
|---|---|---|---|---|
| T26 | 扩展版本管理（多版本共存 / 切换） | 可安装指定版本，查看已安装版本列表 | 4h | ⏳ |
| T27 | 扩展搜索（`list --search <keyword>`） | 支持按关键词过滤远程列表 | 2h | ⏳ |
| T28 | 批量安装（`install` 支持从配置文件读取列表） | 可从 `omni-studio.packages.json` 批量安装 | 3h | ⏳ |
...
- [x] T12 — 实时同步（已完成）
- [x] T13 — fs.watch 兜底方案（已完成）
- [x] T14 — store.ts 自动清理（已完成）
- [x] T15 — 扩展包解压支持（已完成）
- [x] T19 — TUI 体验优化（已完成）
- [x] T20 — 下载缓存机制（已完成）
- [x] T21 — Alert 错误展示与闪退修复（已完成）
- [x] T22 — 生命周期脚本路径调整（已完成）
- [x] T23 — 安装后默认禁用（已完成）
- [x] T24 — dev 分支合并（2025-05-14）Effect Schema + HttpApi + import 路径迁移（已完成，dev 为主开发分支，后续需持续跟踪合并）
- [x] T25 — ripgrep 打包到 CLI（已完成）
- [x] T26 — ripgrep 开发模式修复（已完成）
- [x] T27 — ripgrep 嵌入编译产物（已完成）
- [x] T28 — workflow 恢复单文件上传（已完成）
- [ ] T16 — 安装/更新冲突处理（未开始）
- [ ] T17 — 下载进度条显示（未开始）
- [ ] T18 — Token 自动刷新（未开始）
- [ ] T25 — TUI 键盘快捷键导航（未开始）
